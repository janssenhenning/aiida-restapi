...
class ProcessesQuery(multirow_cls_factory(ProcessQuery, orm.ProcessNode, "processes")):  # type: ignore[misc]
...
def resolve_Process(
    ...
    """Resolution function."""
    return resolve_entity(orm.ProcessNode, info, id, uuid)
...
ProcessQueryPlugin = QueryPlugin(
    "process",
    gr.Field(
        ProcessQuery, id=gr.Int(), uuid=gr.String(), description="Query for a single Node"
    ),
    resolve_Process,
)
ProcessesQueryPlugin = QueryPlugin(
    "processes",
    gr.Field(
        ProcessesQuery, description="Query for multiple Nodes", filters=FilterString()
    ),
    resolve_Processes,
)
